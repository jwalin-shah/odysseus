...
# --- Argument parsing -------------------------------------------------------
# By default the scraper activates Antigravity and drives the UI. In
# --background mode it assumes the user has already positioned the app
# to Settings → Models and just takes a window screenshot without
# touching the foreground.
_parser = argparse.ArgumentParser(add_help=False)
_parser.add_argument("--background", action="store_true",
                     help="skip UI interaction; screenshot the existing Models pane")
_parser.add_argument("--window-id", type=int, default=None,
                     help="screencapture -l<id> target (background mode)")
_args, _rest = _parser.parse_known_args()
BACKGROUND = _args.background
WINDOW_ID = _args.window_id
...
def _find_models_window():
    """Find the Antigravity window id by title (best effort)."""
    script = '''
    tell application "System Events"
        tell process "Antigravity"
            try
    ...
    try:
        ...
        logger.error(f"osascript find window failed: {e}")
    return None
        ...
        if BACKGROUND:
            ...
            if wid is None:
                logger.error("[bg] could not find Antigravity window id; "
                             "open the app first, or pass --window-id")
                return None, None
            logger.info(f"[bg] capturing window id={wid} -> {screenshot1_path}")
            if not _screencapture_window(wid, screenshot1_path):
                return None, None
            time.sleep(0.3)
            try:
                os.replace(screenshot1_path, screenshot2_path)
            except OSError:
                pass
