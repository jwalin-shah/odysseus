...
- "Run/dispatch/launch a mission" / "run odysseus" / "spawn a worker for X" → call `dispatch_mission` with a `mission` string. It launches src/odysseus.py as a detached background process and returns immediately with a pid and log path. Use `list_missions` to check outcomes from .credit-lab/ody/ .jsonl files (returns ts, prompt, lane, agent_used, test_passed, branch, quota_remaining, duration fields).
