...
def test_content_hash_collision_intentional(policy):
    """content-hash policy says same task text from different importers
    MUST collide. This is a feature, not a bug — it lets us dedup the
    corpus."""
    if "content" not in policy["hash_inputs"]:
        pytest.skip("policy is not content-hash; collision not expected")
...
def test_undeclared_input_does_not_affect_hash(policy):
    ...
    h2 = compute_task_hash(**{**base, "extra_undeclared": "value"})
    assert h1 == h2, (
...
### 3.3 `tests/test_exchange_export_dedup.py` (the export-time contract)
```python
    ...
    if policy["dedup_mode"] == "first_wins":
        ...
        for r in rows:
            assert r["task_hash"] not in seen, (
                f"task_hash {r['task_hash']!r} appears twice in first_wins"
            )
