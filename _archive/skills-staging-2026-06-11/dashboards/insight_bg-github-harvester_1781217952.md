...
## 2. Memory Rules (executable, paste-into-supervisor)

```yaml
- id: MR-OD-001
  rule: "Cadence changes to config/miners.json require a 'Measured:' or 'Rollup:'
         line in the commit body. Block merge otherwise. Brief 5 rollups are
         the only valid citation until the eval set lands."
...
- id: MR-OD-004
  rule: "bg-implementer must be disabled by default. When enabled, every write
         ...
         and require a human-in-the-loop approval token from a session other
         than the implementer's own. The non-goal 'no training runs triggered
         by Odysseus' extends to 'no code changes triggered by Odysseus'
...
- id: MR-OD-005
  ...
  source: "1dfa3f5 app.py diff + missing test coverage"
  severity: critical
...
- id: MR-OD-008
  rule: "Five overlapping routers exist (platform/app.py+router.py,
         ...
         Before proposing a new router, grep for these and write a 'why this is
         not a sixth router' paragraph in the PR."
...
## 4. Next Unknown (single, with rationale)
...
5. Ship `tests/test_miners_cadence_baseline.py` asserting the winning cadence.
6. **Block all future `interval_seconds` changes** via MR-OD-001 enforcement.
