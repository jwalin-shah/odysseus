...
### 3.11 `tests/test_brain_skills_outside_click.py`
```python
"""Bug #4002: Pressing outside the editing box in Brain/skills results in
it closing. The fix: the editing box must capture clicks inside without
the outside-click handler firing. This is a 1-line event-stopping fix
plus a test that clicks outside and inside."""
import pytest


def test_outside_click_closes_brain_skills():
    from src.brain.skills import BrainSkills
    bs = BrainSkills()
    bs.open()
    bs.outside_click()
    assert bs.is_open() is False


def test_inside_click_does_not_close_brain_skills():
    from src.brain.skills import BrainSkills
    bs = BrainSkills()
    bs.open()
    bs.inside_click()
    assert bs.is_open() is True, (
        "Issue #4002: clicking inside the editing box must not close it. "
        "The outside-click handler must stop propagation."
    )
```
